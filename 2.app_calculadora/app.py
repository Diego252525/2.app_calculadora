...
def aritmetica():
    if request.method == "POST":
        #valore que rcibo del form n1,n2 son pasados
        num1 = float(request.form.get('n1'))
        num2 = float(request.form.get('n2'))
        #realizamos operacviones aritmeticas
        suma = num1 + num2
        resta = num1 - num2
...
def convertir_divisa():
    if request.method == 'POST':
        pesos = float(request.form.get('pesos'))
        dolar=4200
        dolares = pesos*dolar
        return render_template('divisas.html', total_dolar=dolares,
                                             )
